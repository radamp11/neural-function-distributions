...
class CustomTrainer():
    ...
    def _init_logs(self):
        ...
        self.hyper_gradient_logs = {name: [] for name, _ in
                                    self.function_distribution.hypernetwork.forward_layers.named_parameters()}
    ...
    def _save_training_checkpoint(self, epoch, path):
        torch.save(
            {
                'epoch': epoch,
                'function_distributrion': {
                    'config': self.function_distribution._get_config(),
                    'state_dict': self.function_distribution.state_dict(),
        ...
        for epoch in range(self.epoch, epochs+1):
            ...
            for i, batch in enumerate(dataloader):
                ...
                if i % self.print_freq == 0:
                    if self.r1_weight:
                        print(
                            "{} Iteration {}/{}: generator {:.3f}, discriminator {:.3f}, disc_real {:.3f}, disc_fake {:.3f}, 100 x grad_squared {:.3f}".format(
                                datetime.now(), i + 1, len(dataloader), self.logs["generator"][-1],
                                self.logs["discriminator"][-1], self.logs["disc_real"][-1], self.logs["disc_fake"][-1],
                                100 * self.logs["grad_squared"][-1]))
                    else:
                        print(
                            "{} Iteration {}/{}: generator {:.3f}, discriminator {:.3f}, disc_real {:.3f}, disc_fake {:.3f}".format(
                                datetime.now(), i + 1, len(dataloader), self.logs["generator"][-1],
                                self.logs["discriminator"][-1], self.logs["disc_real"][-1], self.logs["disc_fake"][-1]))
            self._log_epoch_losses(len(dataloader))
            if self.r1_weight:
                print(
                    "\nEpoch {}: generator {:.3f}, discriminator {:.3f}, disc_real {:.3f}, disc_fake {:.3f}, 100 x grad_squared {:.3f}".format(
                        epoch + 1, self.epoch_logs["generator"][-1], self.epoch_logs["discriminator"][-1],
                        self.epoch_logs["disc_real"][-1], self.epoch_logs["disc_fake"][-1],
                        100 * self.epoch_logs["grad_squared"][-1]))
            else:
                print("\nEpoch {}: generator {:.3f}, discriminator {:.3f}, disc_real {:.3f}, disc_fake {:.3f}".format(
                    epoch + 1, self.epoch_logs["generator"][-1], self.epoch_logs["discriminator"][-1],
                    self.epoch_logs["disc_real"][-1], self.epoch_logs["disc_fake"][-1]))
            ...
            if self.save_dir:
                ...
                # self._save_training_checkpoint(epoch=epoch, path=self.save_dir)
                # If model_save_freq is non-zero save intermediate versions of
                # the model
                if self.model_save_freq != 0 and epoch % self.model_save_freq == 0:
                    self._save_training_checkpoint(epoch=epoch, path=self.save_dir)
